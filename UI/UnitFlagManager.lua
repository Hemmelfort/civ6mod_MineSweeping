...
function UnitFlag.SetInteractivity( self )
	...
	--minesweeper--
	self.m_Instance.DigButton:RegisterCallback(Mouse.eLClick, function()
        LuaEvents.MineSweeper_DigPlot()
	end)
	self.m_Instance.MarkButton:RegisterCallback(Mouse.eLClick, function()
        LuaEvents.MineSweeper_MarkPlot()
	end)
end
...
function OnUnitSelectionChanged( playerID : number, unitID : number, hexI : number, hexJ : number, hexK : number, bSelected : boolean, bEditable : boolean )
    ...
    if unitFlag ~= nil then
        unitFlag.m_Instance.DigButton:SetHide(not bSelected);
        unitFlag.m_Instance.MarkButton:SetHide(not bSelected);
    end
